...
| `CATALOG_REQUESTS`                                    | `/tools/requests`의 개발팀 요청 창구. 끄면 화면째 사라집니다                                               |
...
- **리퀘스트** (`tool_requests`): 툴을 직접 만들 수 없는 사람이 곤란한 점을 적고
  개발팀이 대응을 판단합니다. 접수 → 대응 예정 → 대응 중 → 공개, 또는 보류 /
  중복 / 취하. 리퀘스트는 요청자의 所属과 개발팀에게만 보입니다. 리퀘스트에
  연결된 신청이 승인되는 순간 공개 상태가 되므로, 툴 공개가 곧 요청의 종료입니다.
...
- **끌 수 있는 절반**: `CATALOG_SUBMISSIONS`와 `CATALOG_REQUESTS`가 이 배포에서
  ...
  있는지는 별개 문제이고 그쪽은 403입니다. `CATALOG_SUBMISSIONS=admin`은 리퀘스트는
...
| 스위트                             | 지키는 것                                                                                                                                                                         |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tests/Feature/Tools/`             | 카탈로그: 무엇이 나열되는지, 태그 그룹과 개수, 임베드가 외부 https 오리진만 프레임한다는 규칙. 신청 플로우: 초안, 종류별 검증, 취하, 변경·비추천화 신청, 심사 없는 표시 항목 편집. 리퀘스트: 찍히는 所属과 누가 읽을 수 있는지, 끈 기능이 404를 돌려주고 쓸 수 없는 사람에게는 403을 돌려준다는 것 |
| `tests/Feature/Admin/`             | 2단계 승인, 버전 스탬프(같은 분에 두 번째 포함), 슬러그 유일성, 반려. 리퀘스트 대응: 접수·사유가 필요한 보류·중복 병합, 리퀘스트에 연결된 신청의 승인이 공개 상태로 만든다는 것. 관리자 화면: 권한과 所属, 삭제와 완전 삭제, 태그 이름 변경·병합, 실행 이력 정리, 시스템 상태                |
...
| `tests/Feature/DemoSeedTest.php`   | `demo:seed`가 실제 승인 플로우를 통해 공개한다는 것, 리퀘스트도 같은 경로로 등록하고 답이 되는 툴의 공개가 그것을 닫는다는 것, 재실행해도 안전하다는 것, 프로덕션에서는 거부한다는 것 |
