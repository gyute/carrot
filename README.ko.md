...
스크립트 툴은 두 단계가 더 필요합니다. 샌드박스가 기본적으로 꺼져 있기 때문입니다.

```bash
composer sandbox:images       # 컨테이너 이미지 2개를 빌드합니다
```

그다음 `.env`에 `SANDBOX_DRIVER=docker`를 넣습니다. macOS에서는
`SANDBOX_REQUIRE_ROOTLESS=false`도 필요합니다. Docker Desktop은 rootless
데몬이 아니고, 러너가 그런 데몬을 거부하기 때문입니다. 이걸 건너뛰면 스크립트
툴은 전부 `SANDBOX_DRIVER is 'none' on this host`로 실패합니다(링크와 임베드
툴은 영향 없습니다). 기본값이 `none`인 이유는
[샌드박스](#샌드박스)에 있습니다.
...
| `CATALOG_SUBMISSIONS`                                 | 누가 툴을 등록할 수 있는지. `all` / `admin`(개발팀만) / `none`(아무도 못 하고 화면째 사라집니다)           |
...
| `config/catalog.php`                                        | 所属 목록과 두 개의 기능 스위치                                  |
...
- **종류**: `link`는 URL을 엽니다(외부 https 또는 포털 내부 경로). `embed`는 외부
  ...
  스크립트를 실행합니다.
- 카탈로그는 상태·카테고리·所属으로 거를 수 있습니다. 화면에 없는 규칙으로
  숨기지 않습니다. 필터는 非推奨을 뺀 모든 상태가 체크된 상태로 열리므로,
  폐지된 툴도 체크 하나면 나옵니다. 체크는 그때그때 계정에 저장되고
  (`users.catalog_filters`. 여러 번을 한 요청으로 묶고, 먼저 화면을 떠나도
  놓치지 않습니다) 다음에 열면 그대로입니다. 저장 후 이름이 바뀌거나 병합된
  태그 값은 카탈로그 전체를 숨기지 않도록 읽을 때 버립니다.
- **의뢰** (`tool_requests`): 툴을 직접 만들 수 없는 사람이 곤란한 점을 적고
  개발팀이 대응을 판단합니다. 접수 → 대응 예정 → 대응 중 → 공개, 또는 보류 /
  중복 / 취하. 의뢰는 의뢰자의 所属과 개발팀에게만 보입니다. 의뢰에
  연결된 등록 신청이 승인되는 순간 공개 상태가 되므로, 툴 공개가 곧 의뢰의 종료입니다.
- **등록** (`tool_submissions`): 툴 등록, 동작 변경(URL / 스크립트 / 런타임 / 입력),
...
- **권한**: `users.role`은 `member` / `manager` / `admin` 입니다. 신청자 부서의
  **부서 관리자**가 먼저 승인하고, 그다음 **시스템 관리자**가 공개합니다. 관리자는
  ...
  넘어갑니다.
- **퇴직**: 사용자는 삭제하지 않고 퇴직 처리합니다. 받은함·벨·패스키는 지워지고
  로그인할 수 없게 되며, 행은 이름과 주소를 지운 채 남습니다. 그래서 승인 기록은
  누가 했는지를 계속 보여주고 이름만 `退職したユーザー`가 됩니다. 소유하던 툴은
  후임에게 넘어갑니다(소유자 없는 툴은 관리자만 만질 수 있으므로). `/settings/profile`
  에서 스스로 닫는 경우도 같고 툴은 소속 부서 관리자에게 갑니다. 마지막 시스템
  관리자는 닫을 수 없습니다.
- **끌 수 있는 절반**: `CATALOG_SUBMISSIONS`와 `CATALOG_REQUESTS`가 이 배포에서
  두 플로우 중 무엇을 돌릴지 정합니다. 끈 플로우는 '금지'가 아니라 '부재'라서
  라우트가 404를 반환하고 메뉴에서도 사라집니다. 켜진 플로우 안에서 누가 신청할 수
  있는지는 별개 문제이고 그쪽은 403입니다. `CATALOG_SUBMISSIONS=admin`은 의뢰는
  전원에게 받되 등록은 개발팀만 하는 형태가 됩니다.
...
| `/admin/requests`  | 개발팀 대응 큐: 접수·보류·중복 병합·공개 연결   |
| `/admin/users`     | 권한과 所属, 그리고 퇴직자 처리                 |
...
| 스위트                             | 지키는 것                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `tests/Feature/Tools/`             | 카탈로그: 무엇이 나열되는지, 태그 그룹과 개수, 임베드가 외부 https 오리진만 프레임한다는 규칙. 신청 플로우: 초안, 종류별 검증, 취하, 변경·비추천화 신청, 심사 없는 표시 항목 편집. 의뢰: 찍히는 所属과 누가 읽을 수 있는지, 끈 기능이 404를 돌려주고 쓸 수 없는 사람에게는 403을 돌려준다는 것                                                                                                                                 |
| `tests/Feature/Admin/`             | 퇴직 처리: 이력이 익명으로 계속 누가 했는지 보여준다는 것, 받은함과 패스키가 지워진다는 것, 툴이 후임에게 간다는 것, 마지막 관리자는 거부된다는 것. 2단계 승인, 버전 스탬프(같은 분에 두 번째 포함), 슬러그 유일성, 반려. 의뢰 대응: 접수·사유가 필요한 보류·중복 병합, 의뢰에 연결된 등록 신청의 승인이 공개 상태로 만든다는 것. 관리자 화면: 권한과 所属, 삭제와 완전 삭제, 태그 이름 변경·병합, 실행 이력 정리, 시스템 상태 |
...
| `tests/Feature/DemoSeedTest.php`   | `demo:seed`가 실제 승인 플로우를 통해 공개한다는 것, 의뢰도 같은 경로로 등록하고 답이 되는 툴의 공개가 그것을 닫는다는 것, 재실행해도 안전하다는 것, 프로덕션에서는 거부한다는 것                                                                                                                                                                                                                                              |
