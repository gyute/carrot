...
| `tests/Feature/Admin/`             | 2단계 승인, 버전 스탬프(같은 분에 두 번째 포함), 슬러그 유일성, 반려. 의뢰 대응: 접수·사유가 필요한 보류·중복 병합, 의뢰에 연결된 등록 신청의 승인이 공개 상태로 만든다는 것. 관리자 화면: 권한과 所属, 삭제와 완전 삭제, 태그 이름 변경·병합, 실행 이력 정리, 시스템 상태                     |
