...
| `config/github.php`, `app/Support/Github/`                  | 미러: 저장소에 무엇을 어떻게 쓰는지                              |
...
- **퇴직**: 사용자는 삭제하지 않고 퇴직 처리합니다. 받은함·벨·패스키는 지워지고
  ...
  관리자는 닫을 수 없습니다.
- **미러**: `GITHUB_REPOSITORY`와 `GITHUB_TOKEN`을 설정하면 공개된 툴이 private
  저장소의 디렉터리(`tool.json` + 스크립트)가 됩니다. 이름은 `/tools/{ulid}`와 같은
  ULID입니다. 변경마다 커밋이 남고 버전 간
  diff를 볼 수 있습니다. 이벤트가 아니라 **상태**로 동기화합니다 — `tools`에 쓰는
  경로가 8개고 하나씩 거는 방식은 반드시 뒤처지므로, 큐 잡이 행을 다시 읽어 지금
  모습을 씁니다. 이름도 부서도 쓰지 않습니다. git은 추가만 되어서 한 번 넣은 이름은
  되돌릴 수 없기에 사람은 ULID로 적습니다. GitHub이 승인을 막지 않습니다 — 쓰기
  실패는 실패 잡이 되어 `/admin/system`에 보입니다.
...
| `tests/Feature/Github/`            | 설정 전까지 미러가 꺼져 있다는 것, 툴이 바뀌는 모든 경로(소유자 이관 포함)가 미러에 닿는다는 것, 한 변경이 한 커밋이 된다는 것, 차이가 없으면 아무것도 쓰지 않는다는 것, 이름 대신 ULID가 들어간다는 것                                                                                                                                                                                                                        |
