...
スクリプトツールにはもう二手間必要です。サンドボックスは既定で無効だからです。

```bash
composer sandbox:images       # コンテナイメージ 2 つをビルド
```

そのうえで `.env` に `SANDBOX_DRIVER=docker` を書きます。macOS では
`SANDBOX_REQUIRE_ROOTLESS=false` も必要です。Docker Desktop は rootless
デーモンではなく、ランナーはそれを拒否するからです。これを飛ばすと
スクリプトツールは `SANDBOX_DRIVER is 'none' on this host` で失敗します
（リンクと埋め込みのツールは影響を受けません）。既定が `none` である理由は
[サンドボックス](#サンドボックス) にあります。
...
- **種類**: `link` は URL を開き（外部 https かポータル内のパス）、`embed` は外部
  ...
  スクリプトを実行します。
- カタログはステータス・カテゴリ・所属で絞り込めます。画面に出ていない規則で
  隠すことはしません。絞り込みは非推奨以外のステータスにチェックが入った状態で
  開くので、非推奨のツールもチェック 1 つで出せます。チェックはそのつど
  アカウントに保存され（`users.catalog_filters`。まとめて 1 回にし、先に画面を
  離れた場合も取りこぼしません）、次に開くと前回のままです。保存後に名前が
  変わったり統合されたタグの値は、カタログ全体を隠してしまわないよう
  読み込み時に落とします。
- **依頼** (`tool_requests`): ツールを自分で作れない人が困りごとを書き、
  開発チームが対応を判断します。受付中 → 対応予定 → 対応中 → 公開済み、
  あるいは見送り / 重複 / 取り下げ。依頼は依頼者の所属と開発チームだけに
  見えます。依頼に紐づけた申請が承認された時点で公開済みになるので、
  ツールの公開がそのまま依頼のクローズです。
- **登録** (`tool_submissions`): ツールの登録、動作の変更（URL / スクリプト /
...
- **退職**: 利用者は削除ではなく退職処理をします。受信箱・ベル・パスキーは消え、
  サインインできなくなり、行は名前とアドレスを消したうえで残ります。だから承認の
  記録は誰がやったかを示したまま、名前だけが `退職したユーザー` になります。所有
  していたツールは引き継ぎ先に移ります（所有者のいないツールは管理者しか触れなく
  なるため）。`/settings/profile` から自分で閉じる場合も同じで、ツールは所属の部署
  管理者に渡ります。最後のシステム管理者は閉じられません。
- **止められる半分**: `CATALOG_SUBMISSIONS` と `CATALOG_REQUESTS` が、この環境で
  ...
  別問題で、そちらは 403 です。`CATALOG_SUBMISSIONS=admin` は、依頼は全員から
...
| スイート                           | 守っているもの                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tests/Feature/Tools/`             | カタログ: 何が並ぶか、タグのグループと件数、埋め込みが外部 https のオリジンしか表示しないこと。申請フロー: 下書き、種類ごとの検証、取り下げ、変更・非推奨化の申請、審査なしの表示項目編集。依頼: 刻まれる所属と誰が読めるか、止めた機能が 404 を返し、使えない人には 403 を返すこと                                                                                                                                       |
| `tests/Feature/Admin/`             | 退職処理: 履歴は匿名のまま誰がやったかを示し続けること、受信箱とパスキーが消えること、ツールが引き継ぎ先に渡ること、最後の管理者は拒否されること。2 段階承認、バージョンの刻印（同一分の 2 回目を含む）、スラッグの一意性、差し戻し。依頼の対応: 受付・理由付きの見送り・重複統合、依頼に紐づけた申請の承認が公開済みにすること。管理画面: 権限と所属、削除と完全削除、タグの名称変更と統合、実行履歴の削除、システム状態 |
...
| `tests/Feature/DemoSeedTest.php`   | `demo:seed` が実際の承認フローを通して公開すること、依頼も同じ経路で登録し、答えとなるツールの公開がそれを閉じること、再実行しても安全なこと、本番では拒否すること                                                                                                                                                                                                                                                        |
