...
| `config/github.php`, `app/Support/Github/`                  | ミラー: 何をリポジトリに書くか、どう書くか                       |
...
- **ミラー**: `GITHUB_REPOSITORY` と `GITHUB_TOKEN` を設定すると、公開済みツールが
  private リポジトリのディレクトリ（`tool.json` とスクリプト）になります。名前は
  `/tools/{ulid}` と同じ ULID です。変更ごとに
  コミットが残り、版どうしの diff が取れます。イベントではなく状態で同期します
  ——`tools` に書く経路が 8 つあり、ひとつずつ引っ掛ける方式は必ず取りこぼすので、
  キューのジョブが行を読み直して「いまの姿」を書きます。名前も所属も書きません。
  git は追記しかできず、一度入れた名前は取り消せないからで、人は ULID で表します。
  GitHub が承認を止めることはありません——書き込み失敗は失敗ジョブとして
  `/admin/system` に出ます。
...
| `tests/Feature/Github/`            | 設定されるまでミラーが動かないこと、ツールが変わるあらゆる経路（所有者の引き継ぎを含む）がミラーに届くこと、1 変更が 1 コミットになること、差分がなければ何も書かないこと、名前ではなく ULID が入ること                                                                                                                                                                                                                   |
