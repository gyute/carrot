...
| `tests/Feature/Admin/`             | 2 段階承認、バージョンの刻印（同一分の 2 回目を含む）、スラッグの一意性、差し戻し。依頼の対応: 受付・理由付きの見送り・重複統合、依頼に紐づけた申請の承認が公開済みにすること。管理画面: 権限と所属、削除と完全削除、タグの名称変更と統合、実行履歴の削除、システム状態             |
